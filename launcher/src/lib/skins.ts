...
/** Evita que el WebView muestre una skin vieja cacheada. */
export function withCacheBust(url: string | null | undefined, t = Date.now()): string {
  if (!url) return "";
  if (url.startsWith("data:") || url.startsWith("blob:") || url.startsWith("file:")) {
    return url;
  }
  const stripped = url.replace(/([?&])t=\d+/g, "$1").replace(/[?&]$/, "");
  const sep = stripped.includes("?") ? "&" : "?";
  return `${stripped}${sep}t=${t}`;
}

/** URL de cabeza 2D vía Minotar (más fiable en WebView que Crafatar). */
export function minotarHelm(id: string, size = 64): string {
  const clean = id.replace(/-/g, "");
  return withCacheBust(`https://minotar.net/helm/${encodeURIComponent(clean || id)}/${size}.png`);
}

export function minotarBody(id: string, size = 160): string {
  const clean = id.replace(/-/g, "");
  return withCacheBust(`https://minotar.net/armor/body/${encodeURIComponent(clean || id)}/${size}.png`);
}

/** PNG de skin completa (64×64) para visor 3D. */
export function minotarSkin(username: string): string {
  return withCacheBust(`https://minotar.net/skin/${encodeURIComponent(username.trim())}`);
}
