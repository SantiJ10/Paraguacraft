...
import StoreProjectDetail from "@/components/store/StoreProjectDetail.vue";
  ...
  <div class="p-8">
    ...
    >
      <article
        v-for="item in items"
        :key="item.id"
        class="lunar-card flex cursor-pointer gap-4 p-4"
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-2 flex flex-wrap items-center justify-between gap-2">
            ...
            <div class="flex flex-wrap gap-2">
              <BaseButton size="sm" @click.stop="openInstall(item)">Instalar</BaseButton>
              <BaseButton
                v-if="distributionBlocked[item.id] || item.projectUrl"
                size="sm"
                variant="secondary"
                @click.stop="openManualDownload(item)"
              >
    ...
    <StoreProjectDetail
      ...
      @install="openInstall($event); detailItem = null"
    />
  </div>
</template>
