...
class ParaguaCraftLauncher(ctk.CTk):
    def __init__(self):
        ...
        self.optimode_var = ctk.BooleanVar(value=False)
        ...
        self.label_titulo = ctk.CTkLabel(self.frame_main, text="ParaguaCraft Launcher", font=ctk.CTkFont(size=26, weight="bold"))
        self.label_titulo.pack(pady=(15, 10))

        self.entry_usuario = ctk.CTkEntry(self.frame_main, placeholder_text="Introduce tu nombre de jugador", width=300)
        self.entry_usuario.pack(pady=10)

        self.label_version = ctk.CTkLabel(self.frame_main, text="Seleccionar Versión:")
        self.label_version.pack(pady=(5, 5))

        self.frame_lista = ctk.CTkFrame(self.frame_main)
        self.frame_lista.pack(pady=5, fill="x", padx=50)
        
        self.scroll_versiones = ctk.CTkScrollbar(self.frame_lista)
        self.scroll_versiones.pack(side="right", fill="y")
        
        self.lista_versiones = tk.Listbox(self.frame_lista, yscrollcommand=self.scroll_versiones.set, bg="#2b2b2b", fg="white", font=("Arial", 12), selectbackground="#1f538d", height=6)
        self.lista_versiones.pack(side="left", fill="both", expand=True)
        ...
        self.combo_ram = ctk.CTkComboBox(self.frame_opciones, values=["2GB", "4GB", "6GB", "8GB", "16GB", "32GB"], width=80)
        self.combo_ram.pack(side="left")

        self.btn_config = ctk.CTkButton(self.frame_opciones, text="Ajustes Extra", command=self.abrir_config, fg_color="#4a4a4a", width=140)
        self.btn_config.pack(side="right")

        self.btn_carpeta = ctk.CTkButton(self.frame_opciones, text="Carpeta", command=self.abrir_carpeta_minecraft, fg_color="#2b7b4b", width=80)
        self.btn_carpeta.pack(side="right", padx=10)

        self.frame_checks = ctk.CTkFrame(self.frame_main, fg_color="transparent")
        self.frame_checks.pack(pady=5)
        
        ctk.CTkCheckBox(self.frame_checks, text="OpenGL Mesa", variable=self.mesa_var).pack(side="left", padx=10)
        ctk.CTkCheckBox(self.frame_checks, text="Mostrar Consola", variable=self.consola_var).pack(side="left", padx=10)

        self.lbl_estado = ctk.CTkLabel(self.frame_main, text="Listo", text_color="gray")
        self.lbl_estado.pack(pady=5)

        self.barra_progreso = ctk.CTkProgressBar(self.frame_main, width=300)
        self.barra_progreso.pack(pady=5)
        self.barra_progreso.set(0)

        self.boton_jugar = ctk.CTkButton(self.frame_main, text="INICIAR PARAGUACRAFT", command=self.iniciar_juego_thread, font=ctk.CTkFont(size=18, weight="bold"), height=45, width=250)
        self.boton_jugar.pack(pady=(10, 5))

        self.lbl_link = ctk.CTkLabel(self.frame_main, text="Redes Sociales", text_color="#3498db", cursor="hand2", font=("Arial", 11, "underline"))
        self.lbl_link.pack(pady=(0, 10))
        self.lbl_link.bind("<Button-1>", lambda e: webbrowser.open("https://lnk.bio/amin"))
    ...
    def abrir_config(self):
        ...
        vent.title("Ajustes y Optimizaciones")
        ...
        ctk.CTkCheckBox(vent, text="Aplicar gráficos al mínimo", variable=self.opt_var).pack(pady=10)
        ctk.CTkCheckBox(vent, text="Modo Optimode (Mods Optimización para +1.20)", variable=self.optimode_var).pack(pady=5)
    ...
    def iniciar_juego_thread(self):
        ...
        usuario = self.entry_usuario.get() or "Player"
        ...
        self.boton_jugar.configure(state="disabled", text="INICIANDO...")
        self.barra_progreso.set(0)
